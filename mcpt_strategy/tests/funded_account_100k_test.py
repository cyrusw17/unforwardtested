"""
Funded Account Test - $100,000 Starting Balance
Tests SMC strategy with large funded account rules
"""
import sys
...
class FundedAccountRules:
    ...
    def __init__(self, starting_balance: float = 100000):
        ...
        self.min_trading_days = 3
        self.consistency_rule_pct = 0.50
...
def backtest_with_funded_rules(ohlc: pd.DataFrame, starting_balance: float = 100000,
...
def main():
    """Run $100k funded account test"""
    print("="*80)
    print("FUNDED ACCOUNT TEST - $100,000 STARTING BALANCE")
    print("="*80)
    ...
    if rules['account_blown']:
        ...
        print(f"\n💰 PROFIT BREAKDOWN:")
        print(f"   Profit: ${rules['total_profit']:,.2f}")
        print(f"   That's ${rules['total_profit']/39:,.2f} per trading day")
        print(f"   Annualized: ~${rules['total_profit']*(365/195):,.2f}")
    ...
    axes[0].axhline(y=starting_balance, color='gray', linestyle='--', label='Starting Balance')
    axes[0].axhline(y=starting_balance * 1.10, color='green', linestyle='--', label='Profit Target (+10%)')
    axes[0].axhline(y=starting_balance * 0.90, color='red', linestyle='--', label='Max Loss Limit (-10%)')
    axes[0].set_title('$100K Account Balance Over Time', fontsize=14, fontweight='bold')
    ...
    # Drawdown
    drawdown_pct = ((equity_df['equity'] - starting_balance) / starting_balance) * 100
    axes[1].fill_between(equity_df['date'], 0, drawdown_pct, 
                          where=drawdown_pct<0, color='red', alpha=0.3, label='Drawdown')
    axes[1].fill_between(equity_df['date'], 0, drawdown_pct, 
                          where=drawdown_pct>=0, color='green', alpha=0.3, label='Profit')
    axes[1].plot(equity_df['date'], drawdown_pct, 'b-', linewidth=1.5)
    axes[1].axhline(y=-10, color='red', linestyle='--', linewidth=2, label='Max Loss (-10%)')
    axes[1].set_title('Profit/Loss from Starting Balance', fontsize=14, fontweight='bold')
    ...
    output_file = Path(__file__).parent.parent / 'results' / 'funded_account_100k_analysis.png'
    plt.savefig(output_file, dpi=150, bbox_inches='tight')
    print(f"📊 Chart saved to {output_file}")
    
    # Save results
    import json
    results_file = Path(__file__).parent.parent / 'results' / 'funded_account_100k_results.json'
